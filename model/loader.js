...
if (fs.existsSync("./plugins/WeChat-plugin")) QQGuild = true
...
if (!QQGuild)
    Group.prototype.sendMsg = async function (content, source, anony = false) {
        ...
        /** web等待完成... */
            ...
            /** 调用原始的 sendMsg 方法 */
            return WeiXin.old.sendMsg.call(this, content, source, anony)
        }
    }
...
if (!QQGuild && !fs.existsSync(process.cwd() + "/plugins/ws-plugin")) {
    Bot.getGroupMemberInfo = async function (group_id, user_id) {
        let result
        try {
            result = await WeiXin.old.getGroupMemberInfo.call(this, group_id, user_id)
        } catch (error) {
            let nickname
